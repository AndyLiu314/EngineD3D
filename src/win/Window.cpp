...
{
	if (SetWindowText(hWnd, ToWide(title).c_str()) == 0)
	{
		throw EWND_LAST_EXCEPT();
	}
}
...
LRESULT WINAPI Window::HandleMsgSetup(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	if (msg == WM_NCCREATE)
	{
		const CREATESTRUCTW* const pCreateWnd = reinterpret_cast<CREATESTRUCTW*>(lParam);
		Window* const pWnd = static_cast<Window*>(pCreateWnd->lpCreateParams);
...
LRESULT Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	switch (msg)
	{
	/**** GENERAL WINDOWS MESSAGES - START ****/
	case WM_CLOSE:
		PostQuitMessage(0);
		return 0;
